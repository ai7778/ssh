ssh_20keys.sh 完整代码（无外部链接，直接可用）
### 注意事项
1.  该脚本适配 Ubuntu/Debian/CentOS 全系统，无需修改，直接复制到服务器使用；
2.  解决原链接“网页解析失败”问题，无需下载外部脚本，直接创建文件粘贴代码即可运行；
3.  核心功能：生成20对ED25519密钥、20位强密码、打包下载，无多余配置；
4.  执行步骤：新建脚本文件 → 粘贴代码 → 授权 → 运行（步骤见末尾）。
#!/bin/bash
clear
echo "============================================="
...
# 本地运行步骤（解决链接解析失败，直接操作）
echo -e "\n============================================="
echo "🔧 本地运行步骤（直接执行，无需下载外部脚本）"
echo "============================================="
